...
def weekly_paired_bootstrap(frame: pd.DataFrame, champion: pd.DataFrame, challenger: pd.DataFrame, assumptions: EconomicAssumptions, draws: int = 1000, seed: int = 20260903) -> pd.DataFrame:
    if draws < 500:
    ...
    for draw in range(draws):
        ...
        values.append({"draw": draw, "delta_cost": m_h["simulated_total_cost"] - m_c["simulated_total_cost"], "delta_capture": (m_h["fraud_capture"] or 0) - (m_c["fraud_capture"] or 0), "delta_exposure_capture": (m_h["fraud_exposure_capture"] or 0) - (m_c["fraud_exposure_capture"] or 0), "delta_legitimate_intervention": m_h["legitimate_intervention_rate"] - m_c["legitimate_intervention_rate"], "delta_legitimate_block_rate": m_h["legitimate_block_rate"] - m_c["legitimate_block_rate"], "delta_review_rate": m_h["review_rate"] - m_c["review_rate"]})
    result = pd.DataFrame(values)
    rows = []
    for metric in ["delta_cost", "delta_capture", "delta_exposure_capture", "delta_legitimate_block_rate", "delta_review_rate", "delta_legitimate_intervention"]:
