...
from .score_monitor import score_reference_from_series, score_summary
...
def build_baseline(frame: pd.DataFrame, config_dir: Path | None = None) -> dict:
    ...
    score_status = str(reference.get("score_status", pd.Series(["RANKING_ONLY"])).iloc[0]) if len(reference) else "RANKING_ONLY"
    frozen_score = score_reference_from_series(reference["risk_score"], score_status=score_status) if "risk_score" in reference else {"score_status": score_status, "n": 0}
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    write_json(REPORT_DIR / "reference_feature_distributions.json", ref_spec)
    write_json(REPORT_DIR / "reference_score_distribution.json", frozen_score)
    ...
    metadata = {"baseline_id": baseline_id, "reference_scope": "VALIDATION_SELECTION_OR_P7_POLICY_CONFIRM", "reference_row_count": len(reference), "reference_start": reference.transaction_timestamp.min().isoformat(), "reference_end": reference.transaction_timestamp.max().isoformat(), "feature_registry_hash": frame_fingerprint(reference[features]) if features else "EMPTY", "reference_feature_distributions_hash": sha256_file(REPORT_DIR / "reference_feature_distributions.json"), "reference_score_distribution_hash": sha256_file(REPORT_DIR / "reference_score_distribution.json"), "created_at_utc": utc_now(), "oot_used_for_threshold_calibration": False, "raw_rows_persisted": False, "upstream_lineage_status": "NOT_AVAILABLE_UNTIL_PART7_LOCKED"}
    for field in ("model_version", "score_version", "calibration_version", "policy_version", "graph_version", "part5_score_hash", "part7_policy_freeze_hash", "part7_decision_mart_hash"):
        values = reference[field].dropna().astype(str).unique().tolist() if field in reference else []
        metadata[field] = values[0] if values else "NOT_AVAILABLE"
    metadata["frozen_baseline_bundle_hash"] = frame_fingerprint(pd.DataFrame([metadata]))
    write_json(REPORT_DIR / "reference_baseline_metadata.json", metadata)
