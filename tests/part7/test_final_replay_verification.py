...
from src.part7.io import ROOT, write_json


class ReplayVerificationTests(unittest.TestCase):
    def setUp(self):
        self.temp_dir = tempfile.TemporaryDirectory()
        self.report_dir = Path(self.temp_dir.name) / "reports" / "part7"
        self.freeze_path = self.report_dir / "PART7_POLICY_FREEZE.json"
        self.replay_report = self.report_dir / "PART7_REPLAY_VERIFICATION.json"
        self.selected_path = Path(self.temp_dir.name) / "PART7_SELECTED_POLICY.json"
        self.manifest_path = Path(self.temp_dir.name) / "P7_CONFIRMATION_SCOPE_MANIFEST.json"
        self.configs = [ROOT / "config/part7/economic_assumptions.yaml", ROOT / "config/part7/graph_routing_policy.yaml", ROOT / "config/part7/reason_codes.yaml"]
    ...
    def _make_freeze(self, priority_method="SCORE_ONLY"):
        ...
        with patch("src.part7.freeze_policy.git_metadata", return_value=("TESTCOMMIT", False)):
            return freeze_policy(selected, self.configs, "TEST_SCORE", "TEST_MODEL", "TEST_CAL", score_status="PROBABILITY_USABLE", score_path=score, confirmation_scope_hash="CONFIRM_HASH", selected_policy_path=self.selected_path, confirmation_manifest_path=self.manifest_path, report_dir=self.report_dir), score
    ...
    def test_replay_finds_config_in_config_part7(self):
        ...
        with patch("src.part7.replay_contract.git_metadata", return_value=("TESTCOMMIT", False)):
            loaded = load_and_verify_freeze(freeze, score_path=score, selected_policy_path=self.selected_path, confirmation_manifest_path=self.manifest_path, report_dir=self.report_dir)
        ...
        with patch("src.part7.replay_contract.git_metadata", return_value=("TESTCOMMIT", False)):
            for key in ("review_queue_config_sha256", "action_precedence_sha256", "economic_assumption_sha256", "graph_routing_sha256", "reason_code_sha256", "config_bundle_sha256", "code_tree_hash"):
                ...
                with self.assertRaises(RuntimeError, msg=key):
                    load_and_verify_freeze(candidate, score_path=score, selected_policy_path=self.selected_path, confirmation_manifest_path=self.manifest_path, report_dir=self.report_dir)
                candidate.unlink()
    ...
    def test_replay_rejects_score_hash_change(self):
        ...
        with patch("src.part7.replay_contract.git_metadata", return_value=("TESTCOMMIT", False)), self.assertRaises(RuntimeError):
            load_and_verify_freeze(freeze, score_path=other, selected_policy_path=self.selected_path, confirmation_manifest_path=self.manifest_path, report_dir=self.report_dir)
    ...
    def test_replay_rejects_confirmation_manifest_mutation(self):
        ...
        with patch("src.part7.replay_contract.git_metadata", return_value=("TESTCOMMIT", False)), self.assertRaises(RuntimeError):
            load_and_verify_freeze(freeze, score_path=score, selected_policy_path=self.selected_path, confirmation_manifest_path=self.manifest_path, report_dir=self.report_dir)
    ...
    def _make_freeze_ranking(self, priority_method):
        ...
        self.addCleanup(lambda: self.manifest_path.unlink(missing_ok=True))
        with patch("src.part7.freeze_policy.git_metadata", return_value=("TESTCOMMIT", False)):
            return freeze_policy(selected, self.configs, "TEST_RANKING_SCORE", "TEST_MODEL", None, score_status="RANKING_ONLY", score_path=score, confirmation_scope_hash="CONFIRM_HASH", selected_policy_path=self.selected_path, confirmation_manifest_path=self.manifest_path, report_dir=self.report_dir), score
