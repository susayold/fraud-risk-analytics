...
def write_observed_reports(path: Path, stats: dict[str, object]) -> None:
    ...
    policy = {
        "User": ("ENTITY_KEY_ONLY", "Observed synthetic user identifier; use only for entity history keys."),
        "Card": ("ENTITY_KEY_ONLY", "Observed card index; combine with User to form the composite card_key."),
        "Year": ("DERIVE", "Derive transaction_timestamp; do not use as a standalone feature."),
        "Month": ("DERIVE", "Derive transaction_timestamp; do not use as a standalone feature."),
        "Day": ("DERIVE", "Derive transaction_timestamp; do not use as a standalone feature."),
        "Time": ("DERIVE", "Derive transaction_timestamp; same-timestamp history is excluded."),
        "Amount": ("MODEL_OK", "Signed transaction amount; retain raw sign and document amount semantics."),
        "Use Chip": ("MODEL_OK_WITH_MISSINGNESS", "Transaction channel feature; preserve categorical missingness if present."),
        "Merchant Name": ("EDA_ONLY", "Identifier-like merchant value; 100,343 distinct values observed, no merchant dimension supplied."),
        "Merchant City": ("MODEL_OK_WITH_MISSINGNESS", "Categorical location feature with source-coverage caveat."),
        "Merchant State": ("MODEL_OK_WITH_MISSINGNESS", "Structural missingness is measured and retained with an indicator."),
        "Zip": ("MODEL_OK_WITH_MISSINGNESS", "Code-like location field; retain as string and preserve missingness."),
        "MCC": ("MODEL_OK", "Merchant category code treated as categorical."),
        "Errors?": ("EDA_ONLY", "98.4% blank source descriptor; use for EDA only until semantics are confirmed."),
        "Is Fraud?": ("TARGET_ONLY", "Observed synthetic target label; never a model feature."),
    }
    dictionary_rows = []
    for column in columns:
        use_status, reason = policy[column]
        dictionary_rows.append({"table_name": path.stem, "column_name": column, "raw_dtype": "string_or_numeric_in_csv", "standardized_dtype": "date" if column in {"Year", "Month", "Day"} else ("numeric" if column in {"User", "Card", "Amount", "Zip", "MCC"} else "string"), "description": "Observed IBM TabFormer field; semantic mapping retained for Part 2.", "grain": "transaction", "nullable": stats["null_counts"].get(column, 0) > 0, "candidate_key": False, "decision_time_available": "available_at_t0" if column not in {"Is Fraud?"} else "post_event_target", "target_related": column == "Is Fraud?", "pii_like": column in {"User", "Card", "Zip"}, "use_status": use_status, "reason": reason})
    ...
    structural_nulls = {"Merchant State", "Zip", "Errors?"}
    quality_rows = [{"table_name": path.stem, "column_name": column, "check_name": "null_count", "total_rows": stats["rows"], "affected_rows": stats["null_counts"].get(column, 0), "affected_pct": round(stats["null_counts"].get(column, 0) / stats["rows"] * 100, 6), "status": "PASS" if stats["null_counts"].get(column, 0) == 0 else ("DOCUMENTED_LIMITATION" if column in structural_nulls else "FAIL"), "notes": "Empty strings counted as nulls; structural missingness is documented separately." if column in structural_nulls else "Empty strings counted as nulls."} for column in columns]
    ...
    write_csv("key_integrity_report.csv", ["table_name", "key_name", "total_rows", "distinct_keys", "duplicate_key_rows", "orphan_rows", "status", "notes"], [{"table_name": path.stem, "key_name": "User", "total_rows": stats["rows"], "distinct_keys": stats["users"], "duplicate_key_rows": "not_applicable_transaction_grain", "orphan_rows": "not_available_denormalized_source", "status": "PASS_WITH_LIMITATION", "notes": "User repeats by design; no separate user dimension supplied."}, {"table_name": path.stem, "key_name": "User + Card", "total_rows": stats["rows"], "distinct_keys": stats["cards"], "duplicate_key_rows": "not_applicable_transaction_grain", "orphan_rows": "not_available_denormalized_source", "status": "PASS_WITH_LIMITATION", "notes": "Composite card entity key; card repeats across transactions."}])
    ...
    write_csv("leakage_register.csv", ["field_or_feature", "role", "decision_time_rule", "status", "notes"], [{"field_or_feature": "Is Fraud?", "role": "target", "decision_time_rule": "never a feature", "status": "TARGET_ONLY", "notes": "Observed target label."}, {"field_or_feature": "post_event_outcome", "role": "post-event", "decision_time_rule": "exclude", "status": "EXCLUDE", "notes": "Not supplied as a source feature."}, {"field_or_feature": "historical_aggregate", "role": "derived", "decision_time_rule": "strict prior timestamp only", "status": "DERIVE", "notes": "Use point-in-time join with history timestamp < T0."}, {"field_or_feature": "User/Card/Zip", "role": "identifier_like", "decision_time_rule": "controlled entity key or model exclusion", "status": "CONTROLLED", "notes": "Synthetic identifiers; entity-key use is documented and raw identifier claims are not made."}])
    write_csv("split_summary.csv", ["split_name", "date_start", "date_end", "row_count", "fraud_count", "status", "notes"], chronological_splits(stats))
    write_csv("data_issues.csv", ["issue_id", "severity", "status", "area", "description", "recommended_action"], [{"issue_id": "P2-001", "severity": "INFO", "status": "CLOSED", "area": "source", "description": "Raw IBM transaction archive acquired and audited outside repository storage.", "recommended_action": "Keep raw archive on Drive; use temporary processing only."}, {"issue_id": "P2-002", "severity": "MEDIUM", "status": "ACCEPTED_LIMITATION", "area": "keys", "description": "Source has no explicit transaction_id; transaction grain is represented by file row.", "recommended_action": "Use source_row_id as a documented analytical surrogate only; do not present it as a business key."}, {"issue_id": "P2-003", "severity": "MEDIUM", "status": "ACCEPTED_LIMITATION", "area": "schema", "description": "Source is denormalized; standalone user/card/merchant dimension tables are not supplied.", "recommended_action": "Keep a denormalized transaction base and document join limitations."}])
