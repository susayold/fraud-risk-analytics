...
from .build_site_entrypoints import build_root_entrypoint
...
def build() -> dict:
    ...
    source_errors = [f"{row.source_id}: missing" for row in sources.itertuples() if row.status == "NOT_AVAILABLE" and row.source_id in {"part2_summary", "part2_split_summary", "part3_monthly_trend", "part3_channel_risk", "part3_amount_band_risk", "part3_mcc_risk", "part3_entity_concentration", "part4_feature_registry", "part5_summary", "part6_summary", "part7_summary", "part8_summary"}]
    ...
    _write_json(ASSET_DIR / "project_status.json", {"project": "Fraud Risk Analytics", "project_status": statuses["project_status"], "source_precedence": "final report JSON > locked summary > validator > README > HTML", "layers": statuses["layers"]})
    ...
    _write_json(REPORT_DIR / "PART9_FINAL_SUMMARY.json", {"status": "FINAL_PORTFOLIO_RELEASE_LOCKED", "upstream_execution_boundary": "Part 5/7/8 conditional views remain source-driven; Part 6 aggregate graph evidence is locked", "available_charts": [key for key, value in charts.items() if value["status"] == "AVAILABLE"], "blocked_charts": [key for key, value in charts.items() if value["status"] != "AVAILABLE"], "public_boundary": "PASS", "source_count": len(sources), "metric_count": len(metrics), "chart_count": len(charts)})
    commit = subprocess.run(["git", "rev-parse", "HEAD"], cwd=ROOT, capture_output=True, text=True, check=False).stdout.strip() or "UNKNOWN"
    manifest = {"build_timestamp": pd.Timestamp.now("UTC").isoformat(), "code_commit": commit, "source_registry_hash": _sha(REPORT_DIR / "source_manifest.csv"), "summary_hash": _sha(ASSET_DIR / "part9_summary.json"), "charts_hash": _sha(ASSET_DIR / "part9_charts.json"), "status_hash": _sha(ASSET_DIR / "part9_status.json")}
