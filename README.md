...
| 5 | Fraud Rules & Machine Learning | ✅ Locked — FRAUD_CHAMPION_v1 frozen; final OOT evidence available |
...
Part 9 is the flagship recruiter-facing overview. It is built from a deterministic source/metric/status/chart registry, surfaces real Part 2–4 aggregate evidence plus locked Part 6 graph aggregates, and keeps Part 7/8 conditional outputs visibly `INPUT_BLOCKED`, while Part 5/6 locked evidence remains source-driven. The page is `FINAL_PORTFOLIO_RELEASE_LOCKED`; this describes presentation completeness and claim-boundary lock, not enterprise deployment or completion of every upstream execution.
