(() => {
  ...
  fetch('assets/data/part5_summary.json').then(r=>r.ok?r.json():Promise.reject()).then(data=>{
    ...
    set('statusLabel',locked?'MODEL READY · LOCKED':championSelected?'CHAMPION SELECTED · OOT PENDING':'MODELING IN PROGRESS');
    set('statusNote',locked?'Frozen champion and final OOT evidence are available.':championSelected?'Validation candidate selected; OOT remains frozen and unaccessed.':'P5.1 contract is implemented; metrics are withheld until a real offline run completes.');
    set('logisticStatus',championSelected?'CHAMPION CANDIDATE':'PENDING'); set('championStatus',data.champion?.status||'PENDING'); set('championName',data.champion?.model_name||'No champion selected'); set('footerStatus',locked?'MODEL READY · LOCKED':championSelected?'CHAMPION SELECTED · OOT PENDING':'MODELING IN PROGRESS');
  ...
  }).catch(()=>{set('statusLabel','MODEL VALIDATION REVIEW');set('statusNote','Summary unavailable; no model claim is shown.');set('footerStatus','MODEL VALIDATION REVIEW');document.querySelector('.p5-status')?.setAttribute('data-status','review');['pr-chart','calibration-chart','topk-chart','incremental-chart'].forEach(empty);});
