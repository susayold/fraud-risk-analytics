...
def run_policy(frame: pd.DataFrame, config: PolicyConfig, assumptions: EconomicAssumptions, calibrated_probability: bool, label_column: str = "fraud_label", emit_reason_codes: bool = True, evaluation_labels: pd.DataFrame | None = None, queue_config: dict | None = None, precedence_config: dict | None = None, graph_weights: dict[str, float] | None = None) -> tuple[pd.DataFrame, dict]:
    """Compatibility facade; labels must be passed in a separate frame."""
    actions = decide(frame, config, calibrated_probability, emit_reason_codes=emit_reason_codes, queue_config=queue_config, precedence_config=precedence_config, graph_weights=graph_weights)
